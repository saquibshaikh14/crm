...
$.widget( 'evol.structFilter', {
	...
	_setEditorField: function(fid){
		if(this._step<1){
			...
			this._bDel.show();
			if(!this._fList){
				var h='<select id="field"><option disabled selected>Select</option>';
		...
		if(this._step<2){
			...
			switch (fType){
				...
				case fTypes.bool:
					...
					break;
				default:
					h+='<select id="operator"><option disabled selected>Select</option>';
					switch (fType){
						case fTypes.date:
						case fTypes.time:
			...
			if(fType!=fTypes.list && (opVal==evoAPI.sIsNull || opVal==evoAPI.sIsNotNull)){
				...
				if(this._step<3){
					...
					if(fType==fTypes.date){
						editor.find('#value,#value2').datepicker({dateFormat:this.options.dateFormat,changeYear:true, yearRange: "-70:+0"}).attr("autocomplete", "off");
					}
				}
				if(v){
